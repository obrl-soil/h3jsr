...
#' @return By default, a logical vector of length(h3_vertex).
#' @examples
...
#' @param v_num Numeric; the vertex number required. Options are 1-6 inclusive.
#' @param simple Logical; whether to return a vector of outputs or a data frame
#'   containing both inputs and outputs.
#' @return By default, a list of length(h3_address).
#' @examples
...
#' @export
#'
get_cell_vertex <- function(h3_address = NULL, v_num = 1, simple = TRUE) {
...
#' @examples
#' # Get vertex 3 for this cell
#' get_cell_vertices(h3_address = '86be8d12fffffff')
#'
#' @import V8
#' @export
#'
get_cell_vertices <- function(h3_address = NULL, simple = TRUE) {
  ...
  sesh$eval('for (var i = 0; i < evalThis.length; i++) {
            evalThis[i].h3_verts = h3.cellToVertexes(evalThis[i].h3_address);
            };')

  # retrieve the result
  if(simple == TRUE) {
    sesh$get('evalThis')$h3_verts
  } else {
    sesh$get('evalThis')
  }
