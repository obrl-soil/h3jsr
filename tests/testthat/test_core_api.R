...
test_that('h3 to geo boundary returns an appropriate dataset',
          c(
            ...
            expect_is(val1[[1]], 'matrix'),
            expect_equal(ncol(val1[[1]]), 2L),
            expect_is(val1[[1]][1], 'numeric'),
            expect_is(val2, 'data.frame'),
            expect_equal(ncol(val2), 2),
            expect_equal(nrow(val2), 1),
            expect_equal(names(val2), c('h3_address', 'h3_hex')),
            expect_is(val2$h3_hex, 'list'),
            expect_is(val2$h3_hex[[1]], 'matrix'),
            expect_equal(ncol(val2$h3_hex[[1]]), 2L),
            expect_is(val2$h3_hex[[1]][1], 'numeric'),
            # xy checks
            expect_equal(val2$h3_hex[[1]][1,1], 153.0245835),
            expect_equal(val2$h3_hex[[1]][1,2], -27.46896347)
          ))
